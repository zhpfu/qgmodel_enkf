...
subroutine init_fft(kmax)
  ...
  allocate(f(2*kmax+2,2*kmax+2))
  allocate(fr(2*kmax+2,2*kmax+2))
  plan_f=fftw_plan_dft_2d(2*kmax+2,2*kmax+2,f,fr,FFTW_FORWARD,FFTW_PATIENT)
  plan_b=fftw_plan_dft_2d(2*kmax+2,2*kmax+2,f,fr,FFTW_BACKWARD,FFTW_PATIENT)
  deallocate(f,fr)
